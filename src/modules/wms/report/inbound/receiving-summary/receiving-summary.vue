  ...
  <div class="receiving-summary">
    ...
    <div class="grid-100 tablet-grid-100">
      ...
      <div class="grid-100 tablet-grid-100 container">
        <span class="component-title bold">Received Summary</span>
      </div>
      <div class="grid-100 tablet-grid-100 container margin-top-50 ">
        <date-range @selectDateRange="onSelectDateRange" :default-time-from="searchParam.receivedTimeFrom"
          :default-time-to="searchParam.receivedTimeTo"> </date-range>
      </div>
    ...
    <div class="grid-100 tablet-grid-100" v-loading="loading">
      <table class="table-client" v-fixed-head>
        <thead>
          ...
          <tr>
            <th v-for="head in receivingSummaryReports.head" :key="head">

              {{head}}</th>
          </tr>

        </thead>
        <tbody v-for="receivingSummary in currentPageDates" :key="receivingSummary.id">

          <tr>
            <td v-for="head in receivingSummaryReports.head" :key="head">
              <a v-if="isLinkField(head)" href="javascript:void(0)" @click="goInquiryDetail(receivingSummary,head)">
                {{receivingSummary[head]}}
              </a>
      ...
      <pager :totalCount="receivingSummaryReports.data.length" :currentPage="paging.pageNo"
...
<style lang="scss" src="./receiving-summary.scss" />
